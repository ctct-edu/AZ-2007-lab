...
2. **Program クラスの説明を生成**
   ```plaintext
   #editor Program クラスについて説明してください
   ```
...
3. **ProductService.cs の説明を生成**
   ...
   - 同様に説明を生成：
   ```plaintext
   #editor ProductService.cs ファイルについて説明してください
   ```
...
2. **データアクセスクラスの動作説明を生成**
   ```plaintext
   /explain データアクセスクラスがどのように動作するか説明してください
   ```
...
2. **コンテキストの設定**
   - 今回は`#codebase`変数を使用してソリューション全体を対象にします
   - 個別ファイルを追加する必要はありません
...
1. **包括的なプロンプトを入力**
   ```plaintext
   #codebase このコードリポジトリのプロジェクトドキュメントを生成してください。プロジェクトタイトルは「在庫管理システム」を使用してください。ドキュメントには以下のセクションを含めてください：プロジェクト全体の概要、プロジェクト構造、各プロジェクトの役割、アーキテクチャの特徴。すべてのセクションを生のマークダウンとして書式設定してください。プロジェクト構造はインデント付きの箇条書きを使用して表現してください。".vs"、"bin"、"obj"フォルダーは含めないでください。
   ```
...
- **#codebase変数**: プロジェクト全体をコンテキストとして利用
